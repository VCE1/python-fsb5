...
class FSBExtractor:
    ...
    def init_parser(self):
        ...
        parser.add_argument('input_path', nargs='?', default='.',
                            help='path to FSB5 container or directory containing FSB files (defaults to current directory)')
    ...
    def write_to_file(self, filename_prefix, filename, extension, contents):
        directory = os.path.abspath(self.args.output_directory)
    ...
    def handle_directory(self, directory):
        for root, dirs, files in os.walk(directory):
            for file in files:
                if file.lower().endswith('.fsb'):
                    fsb_file = os.path.join(root, file)
    ...
    def run(self, args):
        ...
        if os.path.isdir(self.args.input_path):
            self.handle_directory(self.args.input_path)
        elif os.path.isfile(self.args.input_path):
            with open(self.args.input_path, 'rb') as f:
                self.handle_file(f)
        else:
            self.error('Invalid input path: %s' % self.args.input_path)
            return 1
